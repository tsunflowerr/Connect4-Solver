...
/// Convert a python board to a rust board vector and then a rust Pos object.
pub(crate) fn convert_py_board_to_pos(py_board: &Bound<'_, PyAny>) -> PyResult<Pos> {
    let rows: Vec<Bound<'_, PyAny>> = py_board.iter()?.collect::<Result<_, _>>()?;
    if rows.len() != Pos::N_ROWS {
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(
            format!("Expected {} rows, got {}", Pos::N_ROWS, rows.len()),
        ));
    }

    let mut board = [[0; Pos::N_COLS]; Pos::N_ROWS];
    
    for (user_row, py_row) in rows.iter().enumerate() {
        let cols: Vec<i32> = py_row.extract()?;
        if cols.len() != Pos::N_COLS {
            return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(
                format!("Expected {} columns, got {}", Pos::N_COLS, cols.len()),
            ));
        }
        board[user_row].copy_from_slice(&cols);
    }

    Ok(Pos::from_2d_board(&board))
}

#[pyfunction]
pub fn predict_dummy<'py>(
    py: Python<'py>,
    py_eval_pos_cb: &Bound<'py, PyAny>,
    // py_board: &Bound<'py, PyAny>,
    // max_mcts_iterations: usize,
    // c_exploration: f32,
    // c_ply_penalty: f32,
) -> PyResult<Option<usize>> {
    // let pos = convert_py_board_to_pos(py_board)?;
...
pub fn predict_move<'py>(
    ...
    py.allow_threads(move || {
        let game = interactive_play::InteractivePlay::new_from_pos(
            pos,
            eval_pos,
            max_mcts_iterations,
            c_exploration,
        ...
        Ok(game.predict_random_move(0.0).map(|m| m as usize))
    })
}
