...
impl Pos {
    ...
    // Converts a 2D board to a [Pos] object.
    // The board is represented as a 2D array of i32s where 0 = empty, 1 = player 1, 2 = player 2.
    pub fn from_2d_board(board: &[[i32; Self::N_COLS]; Self::N_ROWS]) -> Self {
        let mut mask = 0u64;
        let mut value = 0u64;
        let mut tmp = 0;
    
        for user_row in 0..Self::N_ROWS {
            for col in 0..Self::N_COLS {
                if board[user_row][col] != 0 {
                    tmp += 1;
                }
            }
        }
    
        let current_turn_value = if tmp % 2 == 0 { 1 } else { 2 };
    
        for user_row in 0..Self::N_ROWS {
            let pos_row = Self::N_ROWS - 1 - user_row;
    
            for col in 0..Self::N_COLS {
                let cell = board[user_row][col];
                if cell != 0 {
                    let idx = Self::_idx_mask_unsafe(pos_row, col);
                    mask |= idx;
        ...
        println!("current_turn_value: {}", current_turn_value);
