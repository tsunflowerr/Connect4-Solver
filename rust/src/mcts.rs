...
impl MctsGame {
    ...
    /// New add
    pub fn predict_random_move(&self, temperature: f32) -> Option<Move> {
        if self.root_pos().is_terminal_state().is_some() {
            println!("🔍 predict_random_move() -> None (terminal state)");
            return None;
        }
        ...
        if temperature == 0.0 {
            let best_move = policy
                ...
                .map(|(i, score)| {
                    println!("🔍 predict_random_move() -> best move: {} with score: {}", i, score);
                    i as Move
                });
            return best_move;
        }
        ...
        let dist = match WeightedIndex::new(policy) {
            Ok(d) => d,
            Err(e) => {
                println!("🔍 predict_random_move() -> Error creating WeightedIndex: {:?}", e);
                return None;
            }
        };
        
        let mov = dist.sample(&mut rng);
        println!("🔍 predict_random_move() -> sampled move: {} (temperature: {})", mov, temperature);
