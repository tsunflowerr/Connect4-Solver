...
impl<E: EvalPosT + Send + Sync + 'static> InteractivePlay<E> {
    ...
    pub fn predict_random_move(&self, temperature: f32) -> Option<Move> {
        let mut state_guard = self.state.lock();
        let mov = state_guard.predict_random_move(temperature);
        drop(state_guard);
        mov
    }
...
impl<E: EvalPosT> State<E> {
    ...
    // New add
    pub fn predict_random_move(&mut self, temperature: f32) -> Option<Move> {
        if self.game.root_pos().is_terminal_state().is_some() {
            return None;
        }
        self.game.predict_random_move(temperature)
    }
